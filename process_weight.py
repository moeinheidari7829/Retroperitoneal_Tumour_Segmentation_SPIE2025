...
for weight_name in os.listdir(weights_path):
    if "3d" not in weight_name:
        dataset_id, _ = os.path.splitext(weight_name)[0].split('_')
        weight_target_folder = os.path.join(
            target_path,
            dataset_name[dataset_id],
            'nnUNetTrainerViLUNet__nnUNetPlans__2d',
            f'fold_{dataset_id}'
        )
        ...
        dataset_id, _, dataset_type = os.path.splitext(weight_name)[0].split('_')
        weight_target_folder = os.path.join(
            target_path,
            dataset_name[dataset_id],
            'nnUNetTrainerViLUNet__nnUNetPlans__3d_fullres',
            f'fold_{dataset_id}'
        )
        os.makedirs(weight_target_folder, exist_ok=True)
